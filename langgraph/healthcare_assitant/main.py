...
st.set_page_config(
    page_title= "RAG Agenet with web search",
    page_icon="🤖",
    layout="wide"
)
...
def create_retriever():
    """Created a retriver from the Chroma DB"""
...
def router_node(state: GraphState) -> str:
    ...
    Question: "{question}" Respond with only 'vector_store' or 'web_search'."""

    llm = ChatGroq(temperature=0, model_name=LLM_MODEL_ID)
    response = llm.invoke(routing_prompt)
    decision = response.content.strip().lower()
    return "web_search" if "web_search" in decision else "vector_store"
