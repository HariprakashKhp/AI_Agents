...
def generate_initial_facts(index: VectorStoreIndex) -> str:
    ...
    try:
        ...
        facts_prompt = PromptTemplate(template=INITIAL_FACTS_TEMPLATE)
...
# print(generate_initial_facts(create_vector_db(split_profile_data(extract_linkedin_profile("", "", True)))))
...
def answer_user_query(index: VectorStoreIndex, user_query: str) -> Any:
    ...
    try:
        ...
        # Create prompt template
        question_prompt = PromptTemplate(template=USER_QUESTION_TEMPLATE)
